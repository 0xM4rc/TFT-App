...
void SpectrogramCalculator::setConfig(const SpectrogramConfig& config) {
    // Crear una copia para validar y corregir
    SpectrogramConfig validatedConfig = config;

    // Validar y corregir fftSize
    if (validatedConfig.fftSize <= 0) {
        qWarning() << "SpectrogramCalculator: fftSize inválido en setConfig, usando 1024";
        validatedConfig.fftSize = 1024;
    }

    // Validar y corregir hopSize
    if (validatedConfig.hopSize <= 0) {
        qWarning() << "SpectrogramCalculator: hopSize inválido en setConfig, usando fftSize/2";
        validatedConfig.hopSize = validatedConfig.fftSize / 2;
    }

    // Validar y corregir sampleRate
    if (validatedConfig.sampleRate <= 0) {
        qWarning() << "SpectrogramCalculator: sampleRate inválido en setConfig, usando 44100";
        validatedConfig.sampleRate = 44100;
    }

    bool windowChanged = (validatedConfig.fftSize != m_config.fftSize ||
                          validatedConfig.windowType != m_config.windowType ||
                          validatedConfig.kaiserBeta != m_config.kaiserBeta ||
                          validatedConfig.gaussianSigma != m_config.gaussianSigma);

    bool freqChanged = (validatedConfig.sampleRate != m_config.sampleRate ||
                        validatedConfig.fftSize != m_config.fftSize);

    // Usar la configuración validada
    m_config = validatedConfig;
...
QVector<SpectrogramFrame> SpectrogramCalculator::processOverlapped(const QVector<float>& samples,
    ...
    if (samples.size() < m_config.fftSize) {
        // Si no hay suficientes muestras, hacer zero-padding
        QVector<float> paddedSamples(m_config.fftSize, 0.0f);
        int copySize = std::min(static_cast<int>(samples.size()), m_config.fftSize);
        for (int i = 0; i < copySize; ++i) {
            paddedSamples[i] = samples[i];
        }

        SpectrogramFrame frame = calculateFrame(paddedSamples, startTimestamp, startOffset);
        frames.append(frame);
        return frames;
    }

    // Calcular número de frames correctamente
    int numFrames = (samples.size() - m_config.fftSize) / m_config.hopSize + 1;

    // Asegurar que procesamos al menos un frame si tenemos suficientes muestras
    if (numFrames < 1 && samples.size() >= m_config.fftSize) {
        numFrames = 1;
    }
    ...
    for (int i = 0; i < numFrames; ++i) {
        ...
        // Verificar que no nos salimos del rango
        if (startIdx + m_config.fftSize > samples.size()) {
            break;
        }

        // Extraer exactamente fftSize muestras
        QVector<float> frameData(m_config.fftSize);
        for (int j = 0; j < m_config.fftSize; ++j) {
            frameData[j] = samples[startIdx + j];
        }
...
QVector<float> SpectrogramCalculator::calculateBlackman(int size) {
    ...
    // Coeficientes corregidos para Blackman
    ...
    for (int i = 0; i < size; ++i) {
        ...
        window[i] = a0 - a1 * std::cos(t) + a2 * std::cos(2.0f * t);

        // Asegurar que el valor está en el rango [0, 1]
        window[i] = std::max(0.0f, std::min(1.0f, window[i]));
    }
